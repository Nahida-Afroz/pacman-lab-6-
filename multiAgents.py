...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def alphaBeta(state, depth, agentIndex, alpha, beta):
            """
            Recursively applies minimax with alpha-beta pruning.
            """
            # Terminal state check (win, lose, max depth)
            if depth == self.depth or state.isWin() or state.isLose():
                return self.evaluationFunction(state)

            # Pacman (Maximizing) vs. Ghosts (Minimizing)
        ...
        def maxValue(state, depth, agentIndex, alpha, beta):
            ...
            """
            v = float('-inf')  # Use the smallest possible value
            best_action = None

            legalActions = state.getLegalActions(agentIndex)
            if not legalActions:  # If no actions, return evaluation
                return self.evaluationFunction(state)

            for action in legalActions:
                successor = state.generateSuccessor(agentIndex, action)
                new_value = alphaBeta(successor, depth, 1, alpha, beta)
            ...
            if depth == 0:
                return best_action  # Return best action at the top level
            return v
        ...
        def minValue(state, depth, agentIndex, alpha, beta):
            ...
            """
            v = float('inf')  # Use the largest possible value
            ...
            if next_agent == state.getNumAgents():  # If last ghost, move to Pacman (next depth)
                next_agent = 0
                depth += 1

            for action in legalActions:
                successor = state.generateSuccessor(agentIndex, action)
                new_value = alphaBeta(successor, depth, next_agent, alpha, beta)
        ...
        # Call alpha-beta pruning with initial values
